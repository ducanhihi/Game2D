...
public class Entity {
	...
	public int chaseTime = 180; // 3 giây đuổi (60 FPS)
	public boolean contactDamage = false; // mặc định: không gây sát thương khi chạm
	public int drawLayer = 1; // 0 = dưới player, 1 = bình thường
	public int attackCooldown = 180; // 3 giây
	...
	public int respawnTime = 300; // ví dụ 10 giây (60fps * 10)
	public int spawnX; // 
	public int spawnY;
	//THUỘC TÍNH NHÂN VẬT
	public int speed;
	...
	public void dropItem(Entity droppedItem) {
	    for (int i = 0; i < gp.obj[1].length; i++) {
	        if (gp.obj[gp.currentMap][i] == null) {
	            ...
	            gp.obj[gp.currentMap][i].worldX = worldX; // Đặt vị trí của vật phẩm
	...
	public void update() {
		...
		if(this.type == type_monster && contactPlayer && contactDamage) {
		    damagePlayer(attack);
		}
		
		//NEU COLLISION LA FALSE THI PLAYER KHONG THE DI CHUYEN
...
//			g2.drawRect(screenX + solidArea.x, screenY + solidArea.y, solidArea.width, solidArea.height);
	...
	public void spawnProjectile(Projectile p, int offset) {
	    p.direction = this.direction;
	    switch(this.direction) {
	        case "up":
	            p.worldX = this.worldX;
	            p.worldY = this.worldY - offset;
	            break;
	        case "down":
	            p.worldX = this.worldX;
	            p.worldY = this.worldY + offset;
	            break;
	        case "left":
	            p.worldX = this.worldX - offset;
	            p.worldY = this.worldY;
	            break;
	        case "right":
	            p.worldX = this.worldX + offset;
	            p.worldY = this.worldY;
